...
public final class CryptoUtils {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        if (args.length == 0 || (args.length == 1 && HELP.equals(args[0]))) {
            ...
            System.out
                    .println("   --hash   password             create hash for password");
            System.out
                    .println("   --verify password digest      verify password for digest");
            System.exit(0);
        }
        ...
        if (HASH.equals(args[0])) {
            if (args.length < 2) {
                throw new IllegalArgumentException(
                        "Error: --hash requires a 'password' argument.");
            }
            final String password = args[1].trim();
            System.out.println(CryptoUtils.getSaltedPassword(password
                    .getBytes("UTF-8")));
        }
        ...
        // User wants to verify an existing password
            ...
            if (args.length < 3) {
                throw new IllegalArgumentException(
                        "Error: --hash requires 'password' and 'digest' arguments.");
            }
            final String password = args[1].trim();
            final String digest = args[2].trim();
            System.out.println(CryptoUtils.verifySaltedPassword(
     ...
     *             20 bytes
     */
    public static byte[] extractPasswordHash(byte[] digest)
            throws IllegalArgumentException {
        if (digest.length < 20) {
            throw new IllegalArgumentException(
     ...
     *             20 bytes
     */
    public static byte[] extractSalt(byte[] digest)
            throws IllegalArgumentException {
        if (digest.length <= 20) {
            throw new IllegalArgumentException(
